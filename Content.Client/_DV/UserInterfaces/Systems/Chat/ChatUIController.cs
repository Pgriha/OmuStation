...
public sealed partial class ChatUIController : IOnSystemChanged<CharacterInfoSystem>
{
    ...
    private static readonly ProtoId<TypingIndicatorPrototype> AdminID = "empty"; // Cheeburbr. Для невидимого адмінчату

// Cheeburbr. Це система хайлайтів Дельти. У Ому ванільна, тож щоб не заважало
//    /// <summary>
    ...
    public void NotifySpecificChatTextChange(ChatSelectChannel selectedChannel)
    {
        ...
        switch (channel)
        {
            ...
            case ChatSelectChannel.Admin:
                _typingIndicator?.ClientAlternateTyping(AdminID);
                break;
            // Cheeburbr end
